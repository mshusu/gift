...
from models.Contrastive import Model
import torch
import torch.nn.functional as F
class Contrastive_LGN(Model):
    @staticmethod
    ...
    def computer(self):
        ...
        users_layer, items_layer = torch.split(embs, [self.user_num, self.item_num])
        #mean
    ...
    def computer_layer(self):
        ...
        users, items = torch.split(light_out, [self.user_num, self.item_num])
        users_layer, items_layer = torch.split(embs, [self.user_num, self.item_num])
        return users, items, users_layer, items_layer
    
    def compute_uu_neigh(self,user_emb):
        
        user_similarity_matrix = torch.matmul(user_emb, user_emb.T)
        k=10
        _, neighbors = torch.topk(user_similarity_matrix, k=k+1, dim=1)

        neighbors = neighbors[:, 1:]

        return neighbors

    def compute_ii_neigh(self,item_emb):
        
        item_similarity_matrix = torch.matmul(item_emb, item_emb.T)
        k=10
        _, neighbors = torch.topk(item_similarity_matrix, k=k+1, dim=1)
